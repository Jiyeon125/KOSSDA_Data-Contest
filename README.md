...
1. '쉬었음' 청년이 **무엇으로 버티는가** — 0-팽창 변수는 보유율·보유자 중앙값으로, 사적 안전망(가족 중심) 구조 확인
2. **생활안전망 격차 검증(H1~H4)** — 부모동거×부채, 가족지원×생활비, 가족 부재 시 도달률(coverage) 공백,
   위험점수(0–5) 중첩·생활안전망 6유형 분화
3. 그 격차의 가장 깊은 곳, **취약의 핵심 축 = '사회적 고립'** 정조준(웰빙↓·도움없음↑, 공식 은둔변수 용량반응)
4. **KGSS 전국 일반화**(취업보다 고립이 웰빙을 가름) + 2022→2024 **재현성·악화 추세** 확인
5. 정책적 **우선 개입 대상**(유형별 차등 + 고립·도움없음) 식별

> 초기엔 '하위 유형 군집화'를 시도했으나 자연 군집이 없어 **폐기**하고(결정로그 D1·D7),
> 내부 위험을 **'고립' 단일 축**으로 재정의했습니다. 자세한 경위는 `docs/analysis_flow.md` 참조.
> 레포 통합·벤치마크 검증 기록은 [`docs/merge_comparison.md`](docs/merge_comparison.md) 참조.
...
│  ├─ merge_comparison.md           # 레포 통합·벤치마크 검증 기록
